...
function F_alt(flu,con,χ)    
    ...
    T01_0 = -8T^4*χ₀*γ^2*v
    T00_1 = χ₁*(-80//3*T^8*ν(γ^2 - 1//4) - 20*T^7*r1*v*γ        - 2*T^6*v^2*t11)
    T01_1 = χ₁*(-80//3*T^8*ν*γ^2*v       - 10*T^7*r1*γ*(1+v^2)  - 2*T^6*t11*v)
    T00_2 = χ₂*T^8*(ν^2(-560*T^4*(γ^2-1//4)) + ν*r1*T^3*(-1120) + ν*t11*T^2*(-112*v^2)
                    + r1^2*T^2*(49 - 217v^2 + 364*γ^2(v^2 -1))
    ...
    T01_2 = χ₂*T^8*(ν^2(-560*T^4*γ^2*v) + ν*r1*T^3*(560*γ*(1-v^2)) + ν*t11*T^2*(-112*v)
                    + r1^2*T^2*8v*(33γ^2(v^2 -1)-21)
                    + r1*t11*T*168*γ*(v^4-1)
                    + t11^2(24*(v^2-1) - 51γ^2*(1-v^2)^2))
    return  [- con[1] + T00_0 + T00_1 + T00_2;
             - con[2] + T01_0 + T01_1 + T01_2;
