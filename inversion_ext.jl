...
function F_alt!(R,flu,con,χ) 
    ...
    #Nota (agregué un 4/3 en T00_2 y NT00_2 que arregla la simétría de los términos no diagonales 3:5 cuando v=0, pero no funciona cuando v!=0. 
    T00_2 = 4/3*χ₂*T^8*(-560*ν^2*T^4*(γ^2-1//4) - 1120ν*r1*T^3*v*γ - 112*ν*t11*T^2*v^2 
                    + r1^2*T^2*(-315 - 217v^2)   
                    - 336*r1*t11*T*v/γ 
                    + t11^2*(-189 + 78*v^2 + 111*v^4)/4)
    ...
    NT00_2 = 4/3*χ₂*T^8*(3.75t11^2 + 49r1^2*T^2 + 140ν^2*T^4 - 31.5v^2*t11^2 + 27.75*v^4*t11^2 
    - 51*γ^2*t11^2 - 217*v^2*r1^2*T^2 - 364*γ^2*r1^2*T^2 - 560*ν^2*γ^2*T^4 - 112*v^2*T^2*t11*ν 
    + 102*γ^2*v^2*t11^2 - 51*γ^2*v^4*t11^2 + 364*γ^2*v^2*r1^2*T^2 - 336*T*r1*t11*v*γ + 336*v^3*T*r1*t11*γ - 1120*T^3*r1*v*γ*ν)
    
    T01_2 = χ₂*T^8*(-560*ν^2*T^4*γ^2*v - 560ν*r1*T^3*γ*(1+v^2) -112ν*t11*T^2*v
                    - 364r1^2*T^2*v
                    - 168r1*t11*T/γ*(v^2+1)
                    - 27*t11^2*v/γ^2)
    
    NT01_2 = χ₂*T^8*(-24t11^2*v - 168r1^2*T^2*v + 24v^3*t11^2 - 168T*r1*t11*γ - 560*T^3*r1*γ*ν - 112T^2*t11*v*ν 
    - 51*γ^2*t11^2*v -364*T^2*γ^2*r1^2*v - 560*T^4*ν^2*γ^2*v + 102*γ^2*v^3*t11^2 - 51*γ^2*v^5*t11^2 
    + 364*γ^2*v^3*r1^2*T^2 + 168*v^4*T*r1*t11*γ - 560*v^2*T^3*r1*γ*ν)
    ...
    R[1] = - con[1] + T00_0 + T00_1 + NT00_2 #T00
