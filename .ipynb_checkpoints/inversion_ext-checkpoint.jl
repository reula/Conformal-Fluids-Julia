...
function F_alt!(R,flu,con,χ) 
    ...
    χ₂ = χ[3]
    γ = (1 - v^2)^(-1//2)
    T00_0 = -S*8T^4*χ₀*(γ^2 - 1//4) #signo cambiado
    T01_0 = -8T^4*χ₀*γ^2*v
    T00_1 = S*χ₁*T^6*(-80//3*T^2*ν*(γ^2 - 1//4) - 20*T*r1*γ*v        - 2*t11*v^2) #signo cambiado
    T01_1 = χ₁*T^6*(-80//3*T^2*ν*γ^2*v          - 10*T*r1*γ*(1+v^2)  - 2*t11*v  )
     
    T00_2 = S*χ₂*T^8*(ν^2*(-560*T^4*(γ^2-1//4)) - ν*r1*T^3*1120v*γ - ν*t11*T^2*112*v^2 
                    - r1^2*T^2*(315 + 217v^2) 
    ...
    A000_1 = 4χ₁*T^5*γ*(1 - 2γ^2) #OK
    A001_1 = S*χ₁*T^5*γ*v*(1 - 6γ^2)  #OK signo cambiado!
    A011_1 = -χ₁*T^5*γ*(1 + 6γ^2*v^2) #OK
    A000_2 = -12χ₂*(T*10ν*γ*  (2γ^2-1)        + 3r1*v*(6γ^2-1)              + t11*(3v^2*γ/T)   )
    A001_2 = -S*12χ₂*(T*10ν*γ*v*(2γ^2-1//3)     + r1*(12γ^2*v^2  + 6γ^2 - 1)  + t11*v*γ/T*(v^2+2)) #signo cambiado
...
function flu_to_abs(flu)
    ...
    return [γ/T; -γ*v/T; ν*T^2*(4γ^2 - 1)/3-2r1*v*γ*T+t11*v^2; -1.0*(4/3*γ^2*v*ν*T^2-r1*γ*T*(1 + v^2)+t11*v); (4*γ^2*v^2 + 1)/3*ν*T^2-2r1*v*γ*T+t11]
end
