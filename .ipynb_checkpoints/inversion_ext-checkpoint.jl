...
function F_alt!(R,flu,con,χ) 
    ...
    A001_1 = -χ₁*T^5*γ*v*(1 - 6γ^2)  #OK signo cambiado!
    A011_1 = -χ₁*T^5*γ*(1 + 6γ^2*v^2) #OK
    A000_2 = -12χ₂*(T*10ν*γ*  (2γ^2-1)        + 3r1*v*(6γ^2-1)              + t11*(3v^2*γ/T)   )
    A001_2 = 12χ₂*(T*10ν*γ*v*(2γ^2-1//3)     + r1*(12γ^2*v^2  + 6γ^2 - 1)  + t11*v*γ/T*(v^2+2)) #signo cambiado
    A011_2 = -12χ₂*(T*10ν*γ*  (2γ^2*v^2+1//3) + r1*v*(6γ^2*v^2 + 12γ^2 + 1) + t11*γ/T*(2v^2+1) )
    R[1] = - con[1] + T00_0 + T00_1 + T00_2 #cambiamos el signo aquí pues son con el índice abajo
...
function flu_to_abs(flu)
    ...
    t11 = flu[5]
    γ = (1 - v^2)^(-1//2)
    return [γ/T; γ*v/T; ν/T^2/3*(4γ^2 - 1)-2r1*v*γ/T+t11*v^2; 4/3*γ^2*v*ν/T^2-r1*γ/T*(1 + v^2)+t11*v; (4*γ^2*v^2 + 1)/3*ν/T^2-2r1*v*γ/T+t11]
end
